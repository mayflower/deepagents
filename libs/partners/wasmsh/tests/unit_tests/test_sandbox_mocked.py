"""Mocked unit tests for WasmshSandbox — no Node or Pyodide assets needed."""
...
class TestInit:
    ...
    def test_passes_initial_files_encoded(self) -> None:
        _, process = _create_sandbox(
            initial_files={"/workspace/a.txt": b"hello"}
        )
        init_params = _stdin_payloads(process)[0]["params"]
    ...
    def test_encodes_string_initial_files(self) -> None:
        _, process = _create_sandbox(
            initial_files={"/workspace/b.txt": "world"}
        )
        init_params = _stdin_payloads(process)[0]["params"]
        files = init_params["initialFiles"]
    ...
    def test_node_not_found_raises(self) -> None:
        with (
            ...
            pytest.raises(FileNotFoundError, match="Node executable not found"),
        ):
            WasmshSandbox()
...
class TestUploadFiles:
    ...
    def test_maps_exception_to_invalid_path(self) -> None:
        ...
        close_ok = json.dumps({"id": 999, "ok": True, "result": {"closed": True}}) + "\n"
