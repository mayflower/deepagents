...
@pytest.mark.skipif(
    ...
    reason="node is required for langchain-wasmsh integration tests",
)
@pytest.mark.skipif(
    not _assets_available,
    reason="Pyodide assets not built (run just build-pyodide && just package-pyodide-runtime)",
)
class TestWasmshSandboxStandard(SandboxIntegrationTests):
    ...
    @pytest.mark.xfail(
        reason="Emscripten VFS does not enforce chmod — permissions are silently ignored",
    )
