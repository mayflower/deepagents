...
Requires: node 20+, built Pyodide assets, ANTHROPIC_API_KEY
"""
...
def test_csv_analysis(sandbox: WasmshSandbox) -> None:
    ...
    sandbox.upload_files([("/workspace/temps.csv", b"city,temp_c\nTokyo,22\nBerlin,15\nCairo,35\nSydney,28\nOslo,5\n")])
...
def test_skill_loading(sandbox: WasmshSandbox) -> None:
    ...
    sandbox.upload_files([("/workspace/skills/md-table-formatter/SKILL.md", skill_md.encode())])

    agent = create_deep_agent(model=MODEL, backend=sandbox, skills=["/workspace/skills"])
    agent.invoke({"messages": [HumanMessage(content=(
...
def test_memory_usage(sandbox: WasmshSandbox) -> None:
    ...
    assert "frankfurt" in deploy["region"].lower() or "eu-central" in deploy["region"].lower()
...
def test_code_execution(sandbox: WasmshSandbox) -> None:
    ...
    sandbox.upload_files([("/workspace/data/numbers.txt", b"42\n17\n88\n3\n56\n91\n25\n64\n10\n73\n")])
